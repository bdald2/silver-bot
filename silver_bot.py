...
- 표시 형식: 금 섹션의 4줄 1세트(카테고리/제품/단위/가격)는 한 줄로 병합
...
def _compact_gold_block(text: str) -> str:
    """금 섹션의 4줄 1세트(카테고리/제품/단위/가격)를 한 줄로 병합.
    ...
    규칙:
    - 카테고리(i) + 제품(i+1)은 공백 없이 붙임
    - 단위(i+2, '당'으로 끝나는 줄)와 가격(i+3, 'XXX원 (XXX)' 패턴) 사이는 공백 1개
    - 4줄 패턴이 아니면 원본 유지 (예: 검인제품/돈당/9/,000원부터 차감 등)
    """
    lines = text.split("\n")
    out = []
    i = 0
    while i < len(lines):
        if (
            ...
            and lines[i].strip()
            and lines[i + 1].strip()
            and lines[i + 2].strip().endswith("당")
            and re.search(r'\d[\d,]*\s*원\s*\([^)]+\)', lines[i + 3].strip())
        ):
            merged = (
                lines[i].strip()
                + lines[i + 1].strip()
...
def get_post_sections(link: str):
    ...
    # 금 섹션만 4줄 → 1줄 병합 (은은 이미 한 줄 형식)
    gold_text = _compact_gold_block(gold_text)
