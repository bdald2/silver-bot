...
- 표시 정책: 알림 발송 시 항상 금·은 두 섹션을 모두 표시 (캐시라도 가져와서 표시).
  변동 방향 표시(↑/↓/=)는 실제로 변경된 쪽에만 붙음.
...
def main() -> None:
    ...
    # 7) 표시용 압축은 가격 추출/저장 이후 적용 (금 섹션만)
    ...
    # head_line은 변경된 쪽만 표시 (변경 없는 쪽은 (변동) 라인 생략)
    ...
    # 8) 캐시 갱신 (변경된 쪽만 — 가격 + 본문 + 제목 + 링크 함께 저장)
